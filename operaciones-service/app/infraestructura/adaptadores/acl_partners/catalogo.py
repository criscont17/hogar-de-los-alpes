...
from app.aplicacion.errores import AdaptadorNoDisponibleError
from app.aplicacion.puertos import AdaptadorDePartner, CatalogoDeAdaptadores
...
class CatalogoDeAdaptadoresEnMemoria(CatalogoDeAdaptadores):
    ...
    def registrar(
        ...
        if adaptador.partner_id in self._adaptadores:
            raise ValueError(f"Adaptador registrado dos veces: {adaptador.partner_id}")
    ...
    def obtener(self, partner_id: str) -> AdaptadorDePartner:
        try:
            ...
            raise AdaptadorNoDisponibleError(
                f"El partner '{partner_id}' no tiene un adaptador de integración"
            ) from None

    def tiene(self, partner_id: str) -> bool:
        return partner_id in self._adaptadores
...
def construir_catalogo(
    *,
    ...
    Ningún recurso de resiliencia se comparte: la caída de un partner abre solo su
    circuito y acumula pendientes solo en su cola.
    """

    catalogo = CatalogoDeAdaptadoresEnMemoria()
