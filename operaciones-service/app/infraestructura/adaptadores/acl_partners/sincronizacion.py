...
class SincronizadorDePartner:
    ...
    - Un hilo por partner (bulkhead): un partner lento o caído solo retrasa su propia cola.
    - Circuit breaker propio: tras fallos consecutivos deja de insistir y falla rápido,
      calibrado por partner porque cada uno tiene un SLA distinto.
    - Cola ordenada de pendientes: lo que no se pudo entregar queda como sincronización
      degradada y se reintenta cuando el circuito vuelve a admitir llamadas.

    Con `asincrono=False` procesa en el mismo hilo que encola.
    """
    ...
    def _bucle(self) -> None:
        ...
        while not self._detener.is_set():
            ...
            # Con el circuito abierto, cualquier intento antes de tiempo falla rápido y uno
            # tardío retrasa la recuperación: se despierta justo cuando admite la prueba.
