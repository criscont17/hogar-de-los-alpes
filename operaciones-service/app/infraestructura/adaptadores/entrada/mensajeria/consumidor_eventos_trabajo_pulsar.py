...
logger = logging.getLogger("operaciones.eventos_trabajo")
...
class ConsumidorDeEventosDeTrabajoPulsar:
    ...
    - Suscripción `Key_Shared`: los hechos de un mismo trabajo (misma clave `trabajo_id`)
      llegan en orden aunque haya varias réplicas de OperacionesBC.
    - Solo procesa mensajes con la propiedad `partner_id`; los de Marketplace se confirman
      sin leer el cuerpo.
    - Rechazos de negocio → ack. Fallas técnicas → nack; tras varias reentregas, Pulsar
      mueve el mensaje a la dead letter queue.
    """
    ...
    def procesar(self, propiedades: Mapping[str, str], cuerpo: bytes) -> bool:
        ...
        if not propiedades.get("partner_id"):
            return True
